...
class UserCreateViewTest(TestCase):
    ...
    def test_create_valid_user(self):
        response = self.client.post(reverse('user-register'), self.valid_payload)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_create_invalid_user(self):
        invalid_payload = self.valid_payload.copy()
        invalid_payload['email'] = ''
        response = self.client.post(reverse('user-register'), invalid_payload)
...
class ContentItemListCreateViewTest(TestCase):
    ...
    def test_create_valid_content_item(self):
        response = self.client.post(reverse('content-list-create'), self.valid_payload)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_create_invalid_content_item(self):
        invalid_payload = self.valid_payload.copy()
        invalid_payload['title'] = ''
        response = self.client.post(reverse('content-list-create'), invalid_payload)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_get_content_items(self):
        response = self.client.get(reverse('content-list-create'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class ContentItemDetailViewTest(TestCase):
    ...
    def test_get_valid_content_item(self):
        response = self.client.get(reverse('content-detail', kwargs={'pk': self.content_item.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_get_invalid_content_item(self):
        response = self.client.get(reverse('content-detail', kwargs={'pk': 9999}))
    ...
    def test_update_content_item(self):
        ...
        response = self.client.put(reverse('content-detail', kwargs={'pk': self.content_item.pk}), updated_payload)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_delete_content_item(self):
        response = self.client.delete(reverse('content-detail', kwargs={'pk': self.content_item.pk}))
...
class ContentItemSearchViewTest(TestCase):
    ...
    def test_search_content_items(self):
        response = self.client.get(reverse('content-search'), {'query': 'Test'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)

    def test_search_no_results(self):
        response = self.client.get(reverse('content-search'), {'query': 'Nonexistent'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
